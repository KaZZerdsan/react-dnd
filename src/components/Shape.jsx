...
const Shape = React.memo((props) => {
  ...
  const handleChange = useCallback((shape, attr, value) => {
  ...
  const handleClose = useCallback(() => {
    setTimeout(() => {if (!ref.current.contains(document.activeElement)) setOpen(false)}, 1)
  }, [ref, setOpen])
    ...
    <Wrapper selected={props.selected} onClick={() => handleSelect(props)}>
      ...
      {open && (
        <ColorPickerWrapper tabIndex={999} ref={ref} onBlur={handleClose}>
          <SketchPicker color={props.color} onChange={e => handleChange(props, 'color', e.hex)} />
        </ColorPickerWrapper>
      )}
    </Wrapper>
  );
